...
const DisputesPage = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      sendTelegramMessage(`Пользователь ${user.username} перешел на страницу 'Споры'`)
    }
  }, [user])

  return (
    <div className='pb-28'>
      <p className={`${inter.className} text-[24px] md:text-[32px] text-[#002269] leading-6 my-[30px]`}>Споры по сделкам</p>

      <div className="bg-[#fbfcfe] rounded-2xl w-full md:w-[50%] space-y-1 border border-[#eef2f9] p-[13px_20px_15px]">
        <p className={`${inter.className} text-[#002269] text-[14px] font-medium mb-2`}>СПОРНЫЕ СДЕЛКИ</p>
        <p className={`font-sans text-[rgb(0 34 105)] text-[20px] font-semibold mb-4`}>0 USDT</p>
        <p className={`${inter.className} text-[#8091b5] text-[14px] font-medium mb-4`}>0 RUB – 0 спорных сделок</p>
      </div>

      <div className="flex flex-col md:hidden gap-2 mt-7 w-full px-1">
