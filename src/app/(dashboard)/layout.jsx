...
const DashboardLayout = ({ children }) => {
  ...
  const handleIconClick = (index) => {
    setActiveIndex(index); // Ikonkaga bosilganda uning indeksini saqlash
  };
  ...
  return (
    <div>
      {/* telefon uchun */}
      <nav className="p-5 mb-7 bg-[#f8f9fc] border-b-[1px] border-[#eef2f9] flex justify-between items-center">
        ...
        {/* <div>
          {active && <FiMenu onClick={() => setActive(!active)} className='text-[28px] cursor-pointer text-[#002269]' />}
          {!active && <AiFillCloseCircle onClick={() => setActive(!active)} className='text-[28px] cursor-pointer ' />}
        </div> */}
      </nav>
      <main className='px-4'>
      ...
      <nav className="fixed bg-white border-t border-[#eef2f9] bottom-0 left-0 right-0  flex justify-between items-center p-5">
